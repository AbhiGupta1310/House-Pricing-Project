...
for train_index, test_index in split.split(housing, housing['income_cat']):
    housing.loc[test_index].drop("income_cat", axis=1).to_csv("input.csv") # Set aside the test data
...
tree_rmses = np.sqrt(-cross_val_score(tree_reg, housing_prepared, housing_labels, scoring="neg_mean_squared_error", cv=10))
# print("Decision Tree CV RMSEs:", tree_rmses)
print("\nCross-Validation Performance (Decision Tree):")
print(pd.Series(tree_rmses).describe())

linear_rmses = np.sqrt(-cross_val_score(lin_reg, housing_prepared, housing_labels, scoring="neg_mean_squared_error", cv=10))
# print("Decision Tree CV RMSEs:", tree_rmses)
print("\nCross-Validation Performance (Linear Regression):")
print(pd.Series(linear_rmses).describe())

random_rmses = np.sqrt(-cross_val_score(forest_reg, housing_prepared, housing_labels, scoring="neg_mean_squared_error", cv=10))
# print("Decision Tree CV RMSEs:", tree_rmses)
print("\nCross-Validation Performance (Random Forest Regression):")
